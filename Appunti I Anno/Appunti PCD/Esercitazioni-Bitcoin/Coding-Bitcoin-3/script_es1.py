...
class Script:
    ...
    def serialize(self):
        # Serializza lo script in bytes
        out = b""
        for cmd in self.cmds:
            if cmd in helpers.OPCODES.values():
                # il dizionario OPCODES è inveritito, cmd è il valore e non la chiave
                # quindi dobbiamo trovare la chiave corrispondente
                cmd = [k for k, v in helpers.OPCODES.items() if v == cmd][0]
                # Se il comando è un opcode, aggiungiamo il byte corrispondente
                out += bytes([cmd])
            else:
                ...
                if length <= 75:
                    ...
                    out += b"\x4c" + bytes([length]) + data
                elif length < 65536:
                    out += b"\x4d" + length.to_bytes(2, "little") + data
                else:
                    out += b"\x4e" + length.to_bytes(4, "little") + data
        return out
...
if __name__ == "__main__":
    ...
    print("Hex Script:", hexscript)
    print("Hex Script Parsed:",script)
    print("Re-Serialize Script hash:",script.serialize().hex())
    print("Script Type:", script.get_type())